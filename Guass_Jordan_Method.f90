...
!      Vector A_Matrix        : An Auxiliary Vector for displacement of two lines    
!       
!      m                      : Coefficient
...
if ( p /= i ) then
Do  counter= 1, n+1, 1
A_Matrix(counter) = Augmented_Matrix(p,counter) ! JA BE JA KARDANE 2 SATR
...
do i = n-1 , 1 ,-1
   do j = n , i+1 , -1
      ...
      Do counter = 1, n+1,1
            Augmented_Matrix(i,counter) = (-m) * Augmented_Matrix(j,counter) + Augmented_Matrix(i,counter)
      End Do
...
Result_Matrix(i) = Augmented_matrix ( i , n+1 ) / Augmented_Matrix ( i , i)
...
End Program Gauss_Jordan_Method
